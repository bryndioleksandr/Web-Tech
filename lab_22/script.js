...
const optionsCategories = selectCategories.options;
...
let renderProducts = (productsArray) => {
    ...
    productsArray.forEach(object => {
        ...
        for(let i = 0; i < optionsCategories.length; i++){
            if(optionsCategories[i].value == category){
                count++;
            }
        }
...
selectCategories.onchange = filterProducts;
...
function filterProducts() {
    ...
    let filteredProducts = array.filter(product => { 
        const matchesCategory = category === 'all' || product.category === category;
        const matchesSearch = product.productName.toLowerCase().includes(searchTerm) || product.description.toLowerCase().includes(searchTerm);
